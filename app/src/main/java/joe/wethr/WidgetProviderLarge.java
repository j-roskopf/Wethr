...
public class WidgetProviderLarge extends AppWidgetProvider {
    DateFormat df_date = new SimpleDateFormat("E, MMM dd");
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            views.setTextViewText(R.id.digitalClock, df_time.format(new Date()));
            views.setTextViewText(R.id.dateWidget, df_date.format(new Date()));
        ...
        Log.d("D", "213546879 in onupdate " + df_time.format(new Date()));
        Log.d("D","213546879 in onupdate " +  df_date.format(new Date()));
    ...
    public void currentWeatherCallback(String url, JSONObject json,
        ...
        try {
            if (json != null) {
                ...
                rightNowWeatherObject.setWeatherCondition(currentWeather
                        .getString("main"));


                rightNowWeatherObject.setTemp(Math.round(Double.parseDouble(main.getString("temp")))
                        + "");
                Log.d("D", "123456789 IN CALLBACK WITH JSON IN WIDGET = " + rightNowWeatherObject.getTemp());
                Log.d("D", "123456789 IN CALLBACK WITH JSON IN WIDGET = " + rightNowWeatherObject.getWeatherCondition());
    ...
    public void setRightNowText(){
        ...
        for (int i = 0; i < widgetLength; i++) {
            ...
            // to the button
            RemoteViews views = new RemoteViews(c.getPackageName(), R.layout.widget_layout_large);
            //views.setOnClickPendingIntent(R.id.button, pendingIntent);

            Log.d("D", "123456789 w object created and pref for temp = " + PreferenceManager.getDefaultSharedPreferences(c).getString(c.getResources().getString(R.string.celcius_or_fahrenheit), c.getResources().getString(R.string.fahrenheit)));

            // To update a label
            views.setTextViewText(R.id.currentTempWidget, rightNowWeatherObject.getTemp() + " " + (char) 0x00B0 + "F");



            views.setTextViewText(R.id.widgetDesc, rightNowWeatherObject.getWeatherCondition());

            int rescIcon = getResourceIcon(rightNowWeatherObject.getIcon());

            views.setImageViewResource(R.id.descImageWidget,rescIcon);
